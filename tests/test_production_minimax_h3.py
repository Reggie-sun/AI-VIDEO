...
def test_recipe_compiles_complete_intent_with_native_remote_grammar() -> None:
    ...
    intent = _complete_intent().model_copy(
        update={
            "dialogue_intent": DialogueIntent(
                ...
                language="zh",
                speaker_id="hero",
                verbatim_text="把产品交给我。",
                start_seconds=1,
                end_seconds=3,
                on_screen=True,
                response_obligation="responds after receiving the product",
    ...
    compiler = AdapterCompilerContract.create(
        compiler_id="minimax-h3-video-compiler", compiler_version="2"
    )
    ...
    assert "Chinese" in compiled.provider_native_prompt
    assert "把产品交给我。" in compiled.provider_native_prompt
    assert provider.resolve(compiled.request).capability_id == h3_module._CAPABILITY_ID

    unsupported = compile_remote_video_prompt(
        _replace_requirement(
            projection,
            generation_intent=intent.model_copy(
                update={
                    "open_state": TypedStateReference(
                        kind=ContinuityStateKind.TYPED_REF,
                        state_ref="canonical-open-state",
                    )
                }
            ),
        ).requirement
    )
    assert isinstance(unsupported, RemoteVideoPromptUnsupported)
    assert unsupported.unsupported_field_paths == ("generation_intent.open_state",)
