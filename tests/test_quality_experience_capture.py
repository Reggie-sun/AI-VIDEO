...
def test_capture_p6_projection_marks_approved_repair_stale_after_policy_drift(
    tmp_path: Path,
) -> None:
    fixture = make_manifest_25_failed_layout_review_fixture(tmp_path)
    before = fixture.load_manifest()
    approved = fixture.committer.record_approved_repair_receipt(
        ...
        fixture.approval,
        expected_manifest_revision=before.manifest_revision,
        attempt_id="q0-p6-policy-drift-approval",
    )
    pointer = approved.active_approved_repair
    assert pointer is not None
    selected = load_production_project(tmp_path / "project.yaml")
    assert selected.qa_policy is not None and selected.render_state is not None
    fresh = reopen_p6_pointers(
        tmp_path, approved, current_render=selected.render_state
    )
    assert next(item for item in fresh if item.kind == "approved_repair").freshness == "fresh"

    replacement_policy = seal_artifact(
        selected.qa_policy.model_copy(
            update={
                "artifact_id": "q0-policy-drift-v2",
                "revision": selected.qa_policy.revision + 1,
                "content_hash": "0" * 64,
                "creation_receipt_id": "q0-policy-drift-v2",
            }
        )
    )
    drifted = fixture.committer.activate_qa_policy(
        replacement_policy,
        expected_manifest_revision=selected.manifest.manifest_revision,
        attempt_id="q0-p6-policy-drift",
    )
    reopened = load_production_project(tmp_path / "project.yaml")
    assert reopened.render_state is not None
    observations = reopen_p6_pointers(
        tmp_path, drifted, current_render=reopened.render_state
    )

    assert next(
        item for item in observations if item.kind == "approved_repair"
    ).freshness == "stale"
...
def test_capture_rejects_fail_continuity_adversarially_promoted_to_candidate(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    _checkpoint(
    ...
    state["candidate_video_asset_ids"] = [state["request"]["output_asset_id"]]
    active_project = manifest["active_project"]
    candidate_project_path = canonical_project_snapshot_path(
        active_project["revision"], active_project["content_hash"]
    )
    candidate_project = dict(active_project, path=candidate_project_path.as_posix())
    source_project = project_root / active_project["path"]
    (project_root / candidate_project_path).parent.mkdir(parents=True, exist_ok=True)
    (project_root / candidate_project_path).write_bytes(source_project.read_bytes())
    attempt["candidate_project"] = candidate_project
    attempt["candidate_registry"] = manifest["active_registry"]
    attempt["candidate_dependency_graph"] = manifest["active_dependency_graph"]
    attempt["candidate_dependency_states_hash"] = "0" * 64
