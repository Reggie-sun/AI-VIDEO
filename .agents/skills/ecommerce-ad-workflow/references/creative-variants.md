...
Master package不要求`creative_variant_matrix`；没有明确受控实验时该字段省略。字段存在时必须至少包含一个variant，且每个 variant 只声明一个 `changed_variable`，可选值为 Hook、opening visual、advertising angle、proof order、CTA、presenter profile、pacing 或 duration cut。另需 `baseline_value`、`variant_value`、complete `held_constants`、`hypothesis` 与 `required_new_assets`。
