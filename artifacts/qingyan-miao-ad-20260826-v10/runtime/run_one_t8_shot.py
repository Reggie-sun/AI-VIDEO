...
TASK_ROOT = REPO / "artifacts/qingyan-miao-ad-20260826-v10"
...
def build_workflow(
    ...
    workflow["11"]["inputs"]["filename_prefix"] = (
        f"development_experiment/qingyan_v10_{shot_id}_seed{seed}"
    )
    workflow["12"] = {
...
def main() -> None:
    ...
    with httpx.Client(timeout=30, trust_env=False, follow_redirects=False) as http:
        ...
        uploaded_name = client.upload_input_bytes(
            f"qingyan-v10-{args.shot_id}-{source_sha[:16]}{source.suffix.lower()}",
            source_bytes,
        )
        last_uploaded_name = None
        if last_frame and last_frame_bytes is not None:
            last_uploaded_name = client.upload_input_bytes(
                f"qingyan-v10-{args.shot_id}-last-{sha256(last_frame_bytes)[:16]}{last_frame.suffix.lower()}",
                last_frame_bytes,
            )
