...
`failure_pattern`：S01 两次独立 `viduq3-pro` I2V、native audio请求都明确要求画面无字幕，
生成物却烧入广播文字。窄范围结论是这两版提示的文字禁止约束未落实；不是“Vidu必然加字幕”。

### Hypothesis

`hypothesis`：口播广播和无字画面之间的分离未被模型可靠遵守。
held constants为同一已登记首帧、同一Provider/model/profile、4秒、1080p、native audio true。
prompt是变化项，且未固定seed；不能推断audio开关造成字幕，不能推断某句提示单独有效或无效。

### Supporting Evidence

`supporting_evidence`：两次独立request/submit和不同MP4；每次的工具及Agent review只计一个单位。
...
`counter_evidence`：检索当前S01 records、本次两份exact Gate与learning目录，未找到同边界
...
Vidu `viduq3-pro` first-frame I2V、4s、1080p、native_audio=true、现有generic compiler表达。
...
`admission_basis`：TWO_INDEPENDENT_ATTEMPTS，两个request identity和不同视频，不以文件数计数。
新的同边界无字幕成功样本应加入counter evidence，避免把现象晋升为必然行为。

### Recommended Action

`recommended_action`：在现有Vidu文档的native audio说明旁增加一条有日期和exact记录链接的
advisory caveat：S01两次原生广播测试中出现烧录字幕，不能把prompt内禁止字幕视为已实现保障；
