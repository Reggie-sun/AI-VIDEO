...
P3 exposes importable `CompositionSpec`, `ResolvedTimeline`, `resolve_composition()`, lifecycle request/path models and durable `render_with_hyperframes()`. `ResolvedTimeline` is the only ordering and timing truth. The accepted surface includes local PNG/JPEG/WebP `STATIC_IMAGE` spans and exact-metadata H.264 MP4 `GENERATED_VIDEO` / `EXISTING_VIDEO` spans, deterministic transforms/z-order/opacity, source trim that exactly covers each resolved Shot, and zero-duration `CUT` transitions. Additive `CompositionSpec` / `ResolvedTimeline` 2.2 accepts only explicitly declared image `graphic_layer_ids` over generated/existing video, bounded commercial graphic text and constrained audited CSS entry/exit treatment；旧2.0/2.1 payload与source bytes保持兼容。Crossfade、arbitrary motion directives、tracking/mask/occlusion/lighting integration、unsupported visual strategies/codecs and Remotion selection fail closed.

`AdCreativePlan` 是广告专属、可seal的上层语义contract；`create_ad_creative_plan()`把accepted typed authoring proposal seal为plan，`compile_ad_creative_handoff()`产生typed Shot proposals、composition requirements与同一`CompositionSpec -> ResolvedTimeline -> HyperFrames`投影，`compile_ad_creative_plan()`保留直接取得composition的兼容入口。`DIALOGUE_SUBTITLE`继续只绑定`CaptionTrack`，其他广告文字不会借用caption layer；exact plan hash、Product Truth/claim reference IDs、safe-area/avoidance/emphasis/brand/synchronization intent与advertising sound cues在projection中保留。`review_ad_creative_plan()`比较完整projected semantics，只返回pure advisory requirements/findings，`production_verdict`固定为`None`，不写Manifest或P6 acceptance。`IN_SCENE_PROVIDER`必须绑定与Shot/asset和plan provenance一致的content-addressed source evidence pointer；真实手持、tracking、mask、遮挡、透视、shadow、lighting与camera matching仍是typed capability gap。

The renderer is exactly project-pinned `hyperframes@0.7.103` with the verified Chrome Headless Shell `152.0.7928.2`. Every non-version command uses root `--json` inside a fresh user/network/PID namespace with a controlled environment and no host fallback. Generated HTML uses root boolean `data-no-timeline`, exact frame/sample metadata and capture-safe CSS opacity keyframes; MP4 spans use stable-ID muted local media with resolved start/duration/media-start, so P4 remains the only audio mixer. URL, media and source-bundle audits run before activation.
...
## Generated Video Providers (P8, offline-accepted; Seedance Mini diagnostic live-success)
...
`SeedanceVideoProvider`按2026-08-19 official snapshot完整覆盖七个current/legacy-callable Model IDs，并拒绝retired/marketing aliases和可扩张profile。它只接受injected `ARK_API_KEY` reference；one-use permit紧邻唯一POST消费，不自动重试POST；poll/fetch绑定exact task/model/endpoint intent，redirect、cross-origin、oversize、MIME/container/brand mismatch均fail closed。默认payload始终显式发送`generate_audio`，但省略false/default/MP4/zero-valued optional defaults；非默认值继续显式发送。`SeedanceAssetMaterializationReceipt`与`SeedanceAssetReferenceResolver`提供already-materialized Ark Console `Active asset-...`的sealed human import，精确绑定local asset ID/SHA/MIME/size与confirmation evidence；它不上传素材、不调用Assets API，也不允许local Registry ID伪装成Ark URI。因Ark已在live请求中把项目虚构写实人物误判为真人，inline `synthetic_photorealistic_person`现在必须在POST和permit消费前fail closed；历史receipt仍可读取，插画与普通非人物synthetic inputs不受影响，并且系统不会自动动漫化或fallback。原offline focused/recovery evidence为`517 passed`，payload regression与相关safety/recovery组合为`108 passed`；identity import bugfix的Seedance/Paid Provider/recovery聚焦组合为`418 passed`。

New generated-video attempts从`video-planner/3`开始：`VideoPlanner`把typed neutral intent、canonical Shot/Scene/Character、continuity以及exact asset/Review evidence唯一派生为plan内嵌、hash-bound `ProviderNeutralVideoRequirement`。Planning freshness boundary只向Shot Router交付verified projection；Router继续独占Provider/profile/capability selection并输出不含prompt的`ProviderBoundVideoRequest`。Selected adapter compiler只能确定性编译native prompt/request/payload，无法完整表达时返回typed unsupported，不能改写Shot、降低requirement、选择其它Provider或fallback。Fake、Local/cloud H3、Hailuo与Seedance mappings只有offline/no-network acceptance；该路径尚未执行新的ComfyUI/Provider submit、媒体生成、subjective quality、Review或Final Acceptance。
...
Project-local MCP configuration exposes `video-analysis` as the default video inspection server for this repository. `.mcp.json` registers the same local stdio server for Claude Code, while `.codex/config.toml` registers it for Codex; native Codex agents that load this repository configuration receive the same eight-tool catalog, and other MCP-capable Claude-compatible clients, including clients routed to domestic models, can reuse the same command. All explicit tool calls, including calls from separate Agent-owned stdio server processes, share the repository's existing analysis worker lock with the background hook and wait rather than overlap. This serializes execution only; process-local Whisper/model caches remain separate and can retain their own memory. Model endpoint and credential selection remain client-owned and are not stored in this MCP or its hooks.
