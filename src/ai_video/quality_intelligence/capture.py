...
def _validate_continuity_terminal_truth(
    bundle,
    attempt,
    state,
    verdict: QaVerdict,
    ...
    candidate_is_exact = (
        ...
        and attempt.candidate_registry is not None
        and attempt.candidate_dependency_graph is not None
        and attempt.candidate_dependency_states_hash is not None
        and state.candidate_video_asset_ids == (state.request.output_asset_id,)
    )
    if state.phase is VideoAttemptPhase.VALIDATE:
        if candidate_is_exact:
            raise _reject(CaptureErrorCode.BINDING_INVALID)
    elif state.phase is VideoAttemptPhase.CANDIDATE:
        if not candidate_is_exact:
            raise _reject(CaptureErrorCode.BINDING_INVALID)
    elif state.phase is VideoAttemptPhase.ACTIVATE:
        if (
            not candidate_is_exact
    ...
    if verdict is QaVerdict.FAIL and state.phase is not VideoAttemptPhase.VALIDATE:
        raise _reject(CaptureErrorCode.BINDING_INVALID)
...
def _build_record(request: PostQaQ0CaptureRequest) -> QualityExperienceRecordV1:
    ...
    if verdict is QaVerdict.NOT_EVALUATED:
        raise _reject(CaptureErrorCode.NOT_READY)
    _validate_continuity_terminal_truth(bundle, attempt, state, verdict)
    ...
    observations = reopen_p6_pointers(
        project_root, bundle.manifest, current_render=bundle.render_state
    )
    p6_state = (
        "not_present"
        if not observations
