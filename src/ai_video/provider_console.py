...
def catalog_runs(
    ...
    *,
    ...
    for path in _walk_files(root, max_depth=max_depth, max_entries=max_entries, scan=scan):
        ...
        if kind == "production":
            try:
                latest_video_attempt_at = _latest_video_attempt_at(
                    _read_production_manifest_nofollow(path.parent)
                )
            except (OSError, ValueError):
                latest_video_attempt_at = None
            if latest_video_attempt_at is not None:
                candidates[workspace]["latest_video_attempt_at"] = latest_video_attempt_at
    ...
    *,
    ...
    for attempt in loaded.manifest.attempts:
        ...
        candidate_media_items = [
            {"role": "candidate", "asset_id": asset_id, "media": _media_projection(
                workspace=workspace, asset=_asset_by_id(assets, asset_id),
                asset_paths=loaded.asset_paths, root=loaded.root, media_map=media_map,
            )}
            for asset_id in candidate_ids[:_MAX_WORKSPACE_MEDIA]
        ]
        ...
        attempts.append(
            {
                ...
                "candidate_media_truncated": len(candidate_ids) > _MAX_WORKSPACE_MEDIA,
