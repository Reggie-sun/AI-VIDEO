...
ROOT = Path(__file__).resolve().parent / "production-s01-v1"
...
def main():
    ...
    for key, cls in types.items():
        ...
        for row in rows if plural else (rows,):
            ...
            prepared.append(writer.prepare_artifact("jieshi-e01-bootstrap", path, payload))
        refs[key] = tuple(group) if plural else group[0]
    project = seal_artifact(models.ProductionProject(
        artifact_id="jieshi-e01-s01-project", revision=1, content_hash="0" * 64,
        creation_receipt_id="jieshi-e01-pending-video-bootstrap",
        source_provenance=(models.SourceReference(
            kind="derived", reference=str(proposal.relative_to(REPO)), content_hash=SOURCE_SHA,
        ),),
        project_id="jieshi-e01-s01", title="界蚀：不存在的终点站／开场首镜", default_language="zh-CN",
    ...
    writer.bootstrap_initial_state(attempt_id="jieshi-e01-bootstrap", project=project,
