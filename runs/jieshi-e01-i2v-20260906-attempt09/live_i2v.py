...
ROOT = RUN / "production-s01-v9"
PREP = RUN / "preparation-v1"
ATTEMPT = "jieshi-e01-s01-vidu-i2v-attempt09"
EXPECTED = "4aa13578f273965fe32ef02c82c90a1c9dbee9f8ea79cbbd0c92038f70c55911"
...
class AuditedTransport:
    ...
    def request(self, request):
        if request.method == "POST":
            ...
            assert request.url == "https://api.vidu.cn/ent/v2/start-end2video"
            ...
            assert payload["audio"] is True and payload["off_peak"] is False
            assert payload["is_rec"] is False
            assert len(payload["images"]) == 2
            for data_uri, binding in zip(payload["images"], self.resolved.image_bindings, strict=True):
            ...
            self.posts += 1
            event("submit_transport_started", body_sha256=hashlib.sha256(request.body).hexdigest(),
                  frame_sha256=[b.asset_sha256 for b in self.resolved.image_bindings])
...
def main(execute):
    ...
    assert resolved.mode.value == "image_to_video" and len(resolved.image_bindings) == 2 and not resolved.media_bindings
    ...
    try:
        ...
        if authorization_path.exists():
            ...
            decision = PaidProviderAuthorizationDecision.create(
                ...
                opt_in_policy_receipt_id="user-jieshi-repair09-authorized-20260906",
                budget_policy_id="jieshi-s01-vidu-repair09-submit-count-one-20260906", budget_currency=profile.currency,
                ...
                egress_authorized=True, egress_policy_receipt_id="user-jieshi-repair09-first-frame-vidu-20260906",
                live_test_authorized=True, live_authorization_receipt_id="user-jieshi-repair09-authorized-20260906",
        ...
        submission = service.submit_once(attempt_id=ATTEMPT, paid_preview=preview, reservation_id="jieshi-s01-vidu-i2v-reservation09")
