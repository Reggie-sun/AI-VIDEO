...
ATTEMPT = "jieshi-s01-authorized-first-frame-v7"
...
def planning(loaded, source):
    prior = VideoPlanningRequest.model_validate_json((RUN / "planning-request-v2.json").read_bytes())
    shot = loaded.shots[0]
    ...
    data.update(
        request_id="jieshi-e01-s01-i2v-plan-v7",
...
def main():
    ...
    policy_data = {"user_instruction": "修复然后执行", "provider": "vidu", "model": "viduq3-pro",
    ...
    output = VideoFlexibleOutputRequirement.model_validate_json((RUN / "output-requirement.json").read_bytes())
    lifecycle = VideoGenerationLifecycleEnvelope(
        generation_id="jieshi-e01-s01-vidu-i2v-v3", target_asset_role="final_visual",
        ...
        input_artifact_ids=(shot.artifact_id, source.asset_id), output_asset_id="jieshi-e01-s01-vidu-video-v3",
    )
    routing = VideoGenerationResolver().resolve_requirement(
        projection=verified, context=context,
        policy=VideoRoutingPolicy(identity=RouterPolicyIdentity(policy_id="jieshi-s01-i2v-v3", policy_version="1", policy_sha256=canonical_sha256(policy_data)),
