...
ATTEMPT = "jieshi-s01-repair-first-frame-attempt02"
...
def planning(loaded, source):
    ...
    intent_data["generation_intent"] = gi.model_copy(update={"scene_continuity": scene})
    intent = ProviderNeutralGenerationIntentProjection.create(**intent_data)
    data = prior.model_dump()
    data.update(
        request_id="jieshi-e01-s01-i2v-plan-attempt02",
...
def main():
    ...
    policy_data = {"user_instruction": "实践；授权", "provider": "vidu", "model": "viduq3-pro",
    ...
    lifecycle = VideoGenerationLifecycleEnvelope(
        generation_id="jieshi-e01-s01-vidu-i2v-attempt02", target_asset_role="final_visual",
        ...
        input_artifact_ids=(shot.artifact_id, source.asset_id), output_asset_id="jieshi-e01-s01-vidu-video-attempt02",
    )
    routing = VideoGenerationResolver().resolve_requirement(
        projection=verified, context=context,
        policy=VideoRoutingPolicy(identity=RouterPolicyIdentity(policy_id="jieshi-s01-i2v-attempt02", policy_version="1", policy_sha256=canonical_sha256(policy_data)),
