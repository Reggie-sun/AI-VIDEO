...
ATTEMPT = "jieshi-s01-repair-first-frame-attempt04"
...
def planning(loaded, source):
    ...
    data.update(
        request_id="jieshi-e01-s01-i2v-plan-attempt04",
...
def main():
    ...
    lifecycle = VideoGenerationLifecycleEnvelope(
        generation_id="jieshi-e01-s01-vidu-i2v-attempt04", target_asset_role="final_visual",
        ...
        input_artifact_ids=(shot.artifact_id, source.asset_id), output_asset_id="jieshi-e01-s01-vidu-video-attempt04",
    )
    routing = VideoGenerationResolver().resolve_requirement(
        projection=verified, context=context,
        policy=VideoRoutingPolicy(identity=RouterPolicyIdentity(policy_id="jieshi-s01-i2v-attempt04", policy_version="1", policy_sha256=canonical_sha256(policy_data)),
