...
ATTEMPT = "jieshi-s01-repair-first-frame-attempt06"
...
def planning(loaded, source):
    ...
    data.update(
        request_id="jieshi-e01-s01-i2v-plan-attempt06",
...
def finish_request(loaded, source, request, verified):
    ...
    lifecycle = VideoGenerationLifecycleEnvelope(
        generation_id="jieshi-e01-s01-vidu-i2v-attempt06", target_asset_role="final_visual",
        ...
        input_artifact_ids=(shot.artifact_id, source.asset_id, endpoint.asset_id), output_asset_id="jieshi-e01-s01-vidu-video-attempt06",
    )
    routing = VideoGenerationResolver().resolve_requirement(
        projection=verified, context=context,
        policy=VideoRoutingPolicy(identity=RouterPolicyIdentity(policy_id="jieshi-s01-i2v-attempt06", policy_version="1", policy_sha256=canonical_sha256(policy_data)),
