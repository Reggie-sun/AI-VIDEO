...
def planning(loaded, source):
    ...
    intent_data["generation_intent"] = gi.model_copy(update={"scene_continuity": scene, "subject_action": gi.subject_action.model_copy(update={"progression": draft["action_progression"]})})
    intent = ProviderNeutralGenerationIntentProjection.create(**intent_data)
    data = prior.model_dump()
    data.update(
        request_id="jieshi-e01-s01-i2v-plan-attempt03",
...
def main():
    ...
    policy_data = {"user_instruction": "继续生成测试", "provider": "vidu", "model": "viduq3-pro",
    ...
    lifecycle = VideoGenerationLifecycleEnvelope(
        generation_id="jieshi-e01-s01-vidu-i2v-attempt03", target_asset_role="final_visual",
        ...
        input_artifact_ids=(shot.artifact_id, source.asset_id), output_asset_id="jieshi-e01-s01-vidu-video-attempt03",
    )
    routing = VideoGenerationResolver().resolve_requirement(
        projection=verified, context=context,
        policy=VideoRoutingPolicy(identity=RouterPolicyIdentity(policy_id="jieshi-s01-i2v-attempt03", policy_version="1", policy_sha256=canonical_sha256(policy_data)),
