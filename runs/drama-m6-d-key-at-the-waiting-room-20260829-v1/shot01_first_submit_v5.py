...
APPROVAL = ROOT / "docs/superpowers/artifacts/drama/b-d0/first-submit/key-at-the-waiting-room-shot-01-v5.accepted.json"
...
def _load_preflight(source_commit: str) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("shot01_v5_preflight", PREFLIGHT_PATH)
...
def _historical_creative_lineage(source_commit: str) -> dict[str, object]:
    ...
    spec = importlib.util.spec_from_file_location("shot01_v5_historical_creative", HISTORICAL_DRIVER_PATH)
...
def _execute(source_commit: str, preflight: ModuleType, selected: dict[str, object], selected_sha256: str, preflight_path: Path, approval_sha256: str, approval: dict[str, object], driver_sha256: str, state: dict[str, object]) -> None:
    ...
    loader = preflight._load(preflight._inventory(source_commit), "shot01_v5_binding", preflight.LOADER_PATH)
    ...
    def runtime_inspector():
        runtime, inspection, _, supervisor = preflight._check_runtime(preflight._load(preflight._inventory(source_commit), "shot01_v5_runtime", preflight.READINESS_PATH), exact["routed"]["profile"])
    ...
    with httpx.Client(trust_env=False, follow_redirects=False, timeout=30, event_hooks={"request": []}) as http:
        ...
        readiness = preflight._load(preflight._inventory(source_commit), "shot01_v5_runtime_provider", preflight.READINESS_PATH)
