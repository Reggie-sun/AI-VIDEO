...
- Shot Continuity provider-neutral technical core已实现：Manifest 2.8由既有`ProductionStateCommitter`保存并恢复terminal-frame extraction/evidence，candidate Registry共同激活generated MP4与derived PNG，P5只沿显式terminal input edge做precise downstream invalidation。Hailuo 2.3与Seedance 2.0 Mini continuity payload只有offline acceptance；本机MiniMax H3虽有checkpoint与native I2V node，但缺sealed complete workflow/profile，local adapter/live/three-lane/subjective acceptance均未完成。不得把模型文件存在、offline tests、crossfade或精确硬切写成continuity quality truth。
