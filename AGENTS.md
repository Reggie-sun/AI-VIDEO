...
- Shot Continuity provider-neutral technical core已实现：Manifest 2.8由既有`ProductionStateCommitter`保存并恢复terminal-frame extraction/evidence，candidate Registry共同激活generated MP4与derived PNG，P5只沿显式terminal input edge做precise downstream invalidation。Local MiniMax H3 `fl2va`现有official-derived sealed API workflow/profile、loopback-only adapter和独立local durable evidence chain；2026-08-19 bounded two-Shot proof验证exact terminal SHA进入下一Shot `first_frame`、candidate/terminal activation/reopen与zero-call replay。Hailuo 2.3与Seedance 2.0 Mini continuity payload仍只有offline acceptance；`ref2va`、three-lane与blinded subjective acceptance未完成。不得把technical live-local、crossfade或精确硬切写成continuity quality truth。
- C2 hard-cut technical contract已实现但尚未live/quality accepted：P7 image request可用strict `character + continuity_terminal`生成独立keyframe，additive Qwen profile不覆盖既有profile hash；P8 `HardCutKeyframeBinding` seal `terminal -> keyframe request/output -> video first_frame` lineage，历史T2V/I2V/C1 hashes保持兼容；P5只沿显式`terminal -> keyframe -> video` closure传播。现有小机器人C1 proof不能解锁Shot Router，Router仍只是spec/plan。
