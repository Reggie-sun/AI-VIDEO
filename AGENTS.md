...
- P8 provider-neutral core、Manifest 2.7 / Registry 2.2、Paid Provider Gate、durable submit/poll/fetch/recovery、candidate activation、Fake 与 MiniMax cloud H3/Hailuo adapters 已进入 local `main`；Seedance offline adapter通过 `c54435f`进入，最终activation/recovery closure通过`a089eac`进入。Hailuo已有多条真实 succeeded/fetched MP4 proof，并在Alice C2 adaptive request上完成canonical activation/recovery/replay；MiniMax cloud H3 live已到达真实API但因余额不足返回`402 / code 1008`，因此该cloud adapter只有offline acceptance而无live success。Seedance Mini在2026-08-19一次授权diagnostic中以one POST到达`succeeded`并fetch H.264 MP4；该请求省略`generate_audio`并产生AAC音轨，只证明cloud connectivity，不证明当前tracked audio-opt-out payload、billing settlement或activation已live accepted。2026-08-20已增加sealed Ark Console `Active` asset identity import/resolver，local Registry ID不能伪装成`asset://`；它不自动上传素材或调用Assets API。所有adapter都保持显式、可删除、非默认且不得remote fallback。
...
- Shot Continuity provider-neutral technical core已实现：Manifest 2.8由既有`ProductionStateCommitter`保存并恢复terminal-frame extraction/evidence，candidate Registry共同激活generated MP4与derived PNG，P5只沿显式terminal input edge做precise downstream invalidation。Local MiniMax H3 `fl2va`现有official-derived sealed API workflow/profile、loopback-only adapter和独立local durable evidence chain；2026-08-19 bounded two-Shot proof验证exact terminal SHA进入下一Shot `first_frame`、candidate/terminal activation/reopen与zero-call replay。Hailuo 2.3已有Alice C2 canonical technical live evidence；Seedance 2.0 Mini continuity已有offline acceptance、pre-submit fail-closed evidence与可信identity import owner，但Alice keyframe尚无真实Ark materialization。`ref2va`、three-lane与blinded subjective acceptance未完成。不得把technical live、crossfade或精确硬切写成continuity quality truth。
- C2 hard-cut technical contract与Alice shared prerequisite已实现；P7 image request可用strict `character + continuity_terminal`生成独立keyframe，P8 `HardCutKeyframeBinding` seal `terminal -> keyframe request/output -> video first_frame` lineage，P5只沿显式closure传播。2026-08-20新的bounded adaptive Hailuo 2.3 request以shared keyframe完成one-submit canonical activation/recovery/replay，人工review为`pass with minor concerns`；Seedance fresh `3 CNY` preview通过，identity import bug已修复，但因Alice尚未取得真实Ark `asset-...`与confirmation evidence仍保持零submit。Local H3 C2 Shot 2、Seedance live与two-provider subjective comparison仍缺，Shot Router仍blocked。
...
- [`.github/workflows/mandatory-gate.yml`](/home/reggie/vscode_folder/AI-VIDEO/.github/workflows/mandatory-gate.yml) 在 targeting `main` 的 pull request 上以 exact PR base/head SHA 运行 `make harness-audit` 与现有 `make harness-verify-range`；check context保持为`mandatory-gate / verify`，作为显式使用 PR 时的验证证据。CI 必须从 GitHub runner 生成自己的 receipt、JUnit 与 logs artifact，MUST NOT 信任开发者提交的本地 receipt，也不得读取 Provider secret 或运行 live Provider、ComfyUI、付费 smoke。
- Workflow 存在或 workflow-only push 不等于 server enforcement。当前 GitHub `main` ruleset 保持 active，允许授权 actor 直接 push，同时阻止 branch deletion 与 force push；PR 与 `mandatory-gate / verify` 不再是直接 push 的前置条件。服务端规则状态必须通过 GitHub API 读取 ruleset enforcement 和 branch protection/ruleset truth 来证明。
